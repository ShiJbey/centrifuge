...
import {
  FamilyRelationshipNodeModel,
} from './FamilyRelationshipNodeModel';
import styled from 'styled-components';
import { DefaultPortLabel } from '@projectstorm/react-diagrams-defaults';
import {
  FAMILY_RELATIONSHIP_NODE_COLOR,
  SELECTION_BORDER_COLOR,
} from '../../constants';

const Node = styled.div<{ background: string; selected: boolean }>`
  background-color: ${(p) => p.background ?? FAMILY_RELATIONSHIP_NODE_COLOR};
  border-radius: 5px;
...
export interface FamilyRelationshipNodeWidgetProps {
  node: FamilyRelationshipNodeModel;
  engine: DiagramEngine;
}

export class FamilyRelationshipNodeWidget extends React.Component<FamilyRelationshipNodeWidgetProps> {
  constructor(props: FamilyRelationshipNodeWidgetProps) {
    super(props);
  }
