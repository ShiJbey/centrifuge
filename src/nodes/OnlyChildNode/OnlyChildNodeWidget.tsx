...
import {
  OnlyChildNodeModel,
} from './OnlyChildNodeModel';
import styled from 'styled-components';
import { DefaultPortLabel } from '@projectstorm/react-diagrams-defaults';
import {
  ONLY_CHILD_NODE_COLOR,
  SELECTION_BORDER_COLOR,
} from '../../constants';

const Node = styled.div<{ background: string; selected: boolean }>`
  background-color: ${(p) => p.background ?? ONLY_CHILD_NODE_COLOR};
  border-radius: 5px;
...
export interface OnlyChildNodeWidgetProps {
  node: OnlyChildNodeModel;
  engine: DiagramEngine;
}

export class OnlyChildNodeWidget extends React.Component<OnlyChildNodeWidgetProps> {
  constructor(props: OnlyChildNodeWidgetProps) {
    super(props);
  }
