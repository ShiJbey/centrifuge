...
import {
  CaptivatedByNodeModel,
} from './CaptivatedByNodeModel';
import styled from 'styled-components';
import { DefaultPortLabel } from '@projectstorm/react-diagrams-defaults';
import {
  CAPTIVATED_BY_NODE_COLOR,
  SELECTION_BORDER_COLOR,
} from '../../constants';

const Node = styled.div<{ background: string; selected: boolean }>`
  background-color: ${(p) => p.background ?? CAPTIVATED_BY_NODE_COLOR};
  border-radius: 5px;
...
export interface CaptivatedByNodeWidgetProps {
  node: CaptivatedByNodeModel;
  engine: DiagramEngine;
}

export class CaptivatedByNodeWidget extends React.Component<CaptivatedByNodeWidgetProps> {
  constructor(props: CaptivatedByNodeWidgetProps) {
    super(props);
  }
