...
class SimulationDatabase {
	...
	insertPerson(person: AttributeMap): void {
		...
		if (modifiedPerson['person/occupations'] && modifiedPerson['person/occupations'].length) {
			const occupations = (modifiedPerson['person/occupations'] as AttributeMap[]).map(
				(occupation, index) => {
			...
			const occupationIDs = _.range(1, occupations.length + 1).map(
				(x) => tx_report.tempids[`${-1 * x}`]
			);
		...
		if (modifiedPerson['person/relationships']) {
			const relationships = Object.values<AttributeMap>(modifiedPerson['person/relationships']).map(
				(relationship, index) => {
			...
			// get the entity IDs from the transaction report
			const relationshipIDs = _.range(1, relationships.length + 1).map(
				(x) => tx_report.tempids[`${-1 * x}`]
			);
